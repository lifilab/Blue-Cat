...
export async function POST(request: Request, { params }: { params: Promise<{ trackingId: string }> }) {
  ...
  try {
    ...
    if (!auth.ok) return NextResponse.json({ error: { code: auth.reason === "mfa_required" ? "MFA_REQUIRED" : auth.reason === "csrf_invalid" ? "CSRF_INVALID" : "UNAUTHORIZED", message: "Acceso no autorizado." }, requestId }, { status: auth.reason === "unauthorized" ? 401 : 403, headers: noStore });
