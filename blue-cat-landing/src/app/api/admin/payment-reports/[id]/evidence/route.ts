...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!auth.ok) return NextResponse.json({ error: { code: auth.reason === "mfa_required" ? "MFA_REQUIRED" : "UNAUTHORIZED", message: "Acceso no autorizado." }, requestId }, { status: auth.reason === "unauthorized" ? 401 : 403, headers: noStore });
