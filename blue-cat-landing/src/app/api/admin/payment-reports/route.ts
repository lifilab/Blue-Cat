...
import { assertCommerceEnabled } from "@/config/commercial-runtime";

const validStatuses = new Set<PaymentReportStatus>(["reported", "under_review", "approved", "rejected"]);
const noStore = { "Cache-Control": "no-store" };

function denied(reason: "unauthorized" | "mfa_required" | "csrf_invalid", requestId: string) {
  const status = reason === "unauthorized" ? 401 : 403;
  const code = reason === "mfa_required" ? "MFA_REQUIRED" : reason === "csrf_invalid" ? "CSRF_INVALID" : "UNAUTHORIZED";
  return NextResponse.json({ error: { code, message: reason === "mfa_required" ? "La operación requiere MFA." : "Acceso administrativo no autorizado." }, requestId }, { status, headers: noStore });
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const auth = await authenticateAdmin(request, { csrf: true });
    if (!auth.ok) return denied(auth.reason, requestId);
    ...
    const code = safeErrorCode(error);
    if (code === "COMMERCE_NOT_ENABLED") return NextResponse.json({ error: { code, message: "El canal comercial no está habilitado." }, requestId }, { status: 503, headers: noStore });
