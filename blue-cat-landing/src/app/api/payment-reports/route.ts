...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!/^\d+$/.test(rawContentLength)) return NextResponse.json({ error: { code: "CONTENT_LENGTH_REQUIRED", message: "No fue posible validar el tamaño del comprobante." }, requestId }, { status: 411 });
    ...
    const clientErrors: Record<string, { status: number; message: string }> = {
      ...
      COMMERCE_NOT_ENABLED: { status: 503, message: "El canal de pagos todavía no está habilitado." },
    };
