...
function num(n) {
  ...
  var s = String(n).trim().replace(/\$/g, '').replace(/\s/g, '');
  ...
  api('productos', { search: search, id_categoria: num(cat), id_marca: num(marca), estado: estado, limit: 200 }, function(r) {
    ...
    tbody.innerHTML = r.items.map(function(p) {
      ...
      var actions = '<button class="btn btn-outline btn-xs" onclick="showProductDetail(' + p.id_producto + ')" title="Ver detalle"><i class="fas fa-eye"></i></button> ' +
        '<button class="btn btn-outline btn-xs" onclick="showProductEditForm(' + p.id_producto + ')" title="Editar"><i class="fas fa-edit"></i></button> ' +
        '<button class="btn btn-success btn-xs" onclick="showStockUpdate(' + p.id_producto + ')" title="Actualizar stock"><i class="fas fa-cubes"></i></button> ' +
        '<button class="btn btn-warning btn-xs" onclick="showPriceUpdate(' + p.id_producto + ')" title="Actualizar precio"><i class="fas fa-dollar-sign"></i></button>';
...
function quickStockSave() {
  ...
  api('movimiento_crear', {
    ...
    _stockProduct = null;
    loadProductos(); loadMovimientos(); loadDashboard();
...
function showStockUpdate(id) {
  api('producto', { id: id }, function(p) {
    var stockBodegas = p.stock_bodegas || [];
    var selectedBodega = stockBodegas.length ? stockBodegas[0].id_bodega : '';
    var stockActual = stockBodegas.length ? num(stockBodegas[0].disponible) : num(p.cantidad);
    openModal(
      '<div class="modal-body"><h3><i class="fas fa-cubes"></i> Actualizar Stock</h3>' +
      '<form onsubmit="saveStockUpdate(event,' + p.id_producto + ')"><div class="form-grid">' +
        '<div class="full"><label>Producto</label><div style="font-size:14px;padding:8px 0;font-weight:600;">' + esc(p.nombre_producto) + '</div></div>' +
        '<div><label>Bodega</label><select id="upd-stock-bodega"></select></div>' +
        '<div><label>Stock actual</label><input type="text" id="upd-stock-actual" value="' + fmt(stockActual) + '" readonly></div>' +
        '<div class="full"><label>Nuevo stock *</label><input type="text" id="upd-stock-valor" value="' + stockActual + '" required inputmode="decimal"></div>' +
      '</div><button type="submit" class="btn btn-success" style="width:100%;justify-content:center;margin-top:12px;"><i class="fas fa-save"></i> Guardar stock</button></form></div>'
    );
    loadBodegasSelect('upd-stock-bodega', selectedBodega);
  });
}

function saveStockUpdate(e, id) {
  e.preventDefault();
  var stock = num(document.getElementById('upd-stock-valor').value);
  var idBodega = num(document.getElementById('upd-stock-bodega').value);
  if (stock < 0) { toast('El stock no puede ser negativo', 'error'); return; }
  api('stock_set', { id_producto: id, id_bodega: idBodega, stock: stock }, function(r) {
    toast(r.mensaje || 'Stock actualizado');
    closeModal();
    loadProductos();
    loadStock();
    loadDashboard();
...
function showPriceUpdate(id) {
  api('producto', { id: id }, function(p) {
    openModal(
      '<div class="modal-body"><h3><i class="fas fa-dollar-sign"></i> Actualizar Precio</h3>' +
      '<form onsubmit="savePriceUpdate(event,' + p.id_producto + ')"><div class="form-grid">' +
        '<div class="full"><label>Producto</label><div style="font-size:14px;padding:8px 0;font-weight:600;">' + esc(p.nombre_producto) + '</div></div>' +
        '<div><label>Precio actual</label><input type="text" value="$' + fmt(p.precio_venta) + '" readonly></div>' +
        '<div><label>Nuevo precio *</label><input type="text" id="upd-price-valor" value="' + num(p.precio_venta) + '" required inputmode="decimal"></div>' +
      '</div><button type="submit" class="btn btn-warning" style="width:100%;justify-content:center;margin-top:12px;"><i class="fas fa-save"></i> Guardar precio</button></form></div>'
    );
  });
}

function savePriceUpdate(e, id) {
  e.preventDefault();
  var precio = num(document.getElementById('upd-price-valor').value);
  if (precio < 0) { toast('El precio no puede ser negativo', 'error'); return; }
  api('precio_actualizar', { id_producto: id, precio_venta: precio }, function(r) {
    toast(r.mensaje || 'Precio actualizado');
    closeModal();
    loadProductos();
    loadDashboard();
  });
...
function showQuickPrice() {
  openModal(
    '<div class="modal-body"><h3><i class="fas fa-dollar-sign"></i> Actualizar Precio</h3>' +
    '<form onsubmit="saveQuickPrice(event)"><div class="form-grid">' +
      '<div class="full"><label>Producto *</label><select id="quick-price-producto" required></select></div>' +
      '<div class="full"><label>Nuevo precio *</label><input type="text" id="quick-price-valor" required inputmode="decimal" placeholder="0"></div>' +
    '</div><button type="submit" class="btn btn-warning" style="width:100%;justify-content:center;margin-top:12px;"><i class="fas fa-save"></i> Guardar precio</button></form></div>'
  );
  loadProductosSelect('quick-price-producto');
}

function saveQuickPrice(e) {
  e.preventDefault();
  var id = num(document.getElementById('quick-price-producto').value);
  var precio = num(document.getElementById('quick-price-valor').value);
  if (!id) { toast('Seleccione un producto', 'error'); return; }
  if (precio < 0) { toast('El precio no puede ser negativo', 'error'); return; }
  api('precio_actualizar', { id_producto: id, precio_venta: precio }, function(r) {
    toast(r.mensaje || 'Precio actualizado');
    closeModal();
