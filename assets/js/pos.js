  ...
  apiGet({ accion: 'permisos_usuario' }, function(d) {
    ...
    if (Array.isArray(permisos)) {
      ...
      permisos.forEach(function(p) {
        map[p.modulo] = (p.acciones || []).map(function(a) {
          return typeof a === 'string' ? a : a.accion;
        });
      });
      permisos = map;
    }
...
function api(method, data, cb) {
  ...
  xhr.onload = function () {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      try { var e = JSON.parse(xhr.responseText); toast(e.message || e.mensaje || e.error || 'Error', 'err'); }
...
function loadCajaState() {
  apiGet({ accion: 'caja_estado' }, function (d) {
    if (d.caja) {
      d.id_caja = d.id_caja || d.caja.id_caja;
      d.monto_actual = d.monto_actual || d.caja.monto_actual || 0;
      d.codigo = d.codigo || d.caja.codigo || d.caja.nombre || '';
    }
    if (d.caja && !d.estado) {
      d.estado = d.abierta ? (d.caja.estado || 'ABIERTA') : 'CERRADA';
      d.monto_actual = d.caja.monto_actual || 0;
      d.codigo = d.caja.codigo || d.caja.nombre || '';
    }
    cajaState = d;
    if (d.estado === 'ABIERTA') {
      $('caja-status').innerHTML = '<span class="badge badge-ABIERTA">Abierta</span>';
      $('caja-monto').textContent = fm(d.monto_actual);
      $('caja-codigo').textContent = d.codigo || d.nombre || 'Caja';
...
function showAbrirCaja() {
  var nombreUsuario = sessionStorage.getItem('nombre_completo') || sessionStorage.getItem('user_name') || 'Usuario';
  var m = showModal(`
...
function abrirCaja() {
  ...
  apiPost({
    ...
    var codigo = d.codigo || (d.caja && d.caja.codigo) || 'Caja';
    toast('<i class="fas fa-check-circle"></i> Caja abierta: ' + codigo);
    closeModal();
    loadCajaState();
  });
}
...
function handleBarcode() {
  ...
  for (var i = 0; i < products.length; i++) {
    if (products[i].codigo_de_barras === q) {
      addToCart(products[i].id_producto, products[i].nombre_producto, products[i].precio_venta, products[i].codigo_de_barras, products[i].cantidad);
      $('search-input').value = '';
      currentCat = savedCat;
      return;
    }
...
function addToCart(id, name, price, sku, stock) {
  ...
  cart.push({ id: id, name: name, price: price, sku: sku || '', cant: 1, stock: stk });
  renderCart();
...
function applyPromo() {
  ...
  var items = cart.map(function (c) { return { id_producto: c.id, cantidad: c.cant, precio: c.price, precio_unitario: c.price }; });
  var total = items.reduce(function (s, it) { return s + it.precio * it.cantidad; }, 0);

  apiPost({ accion: 'promocion_validar', codigo: code, items: items, total: total, subtotal: total }, function (d) {
    if (d.valida === false) {
      toast(d.message || 'Promocion no valida', 'err');
      return;
    }
    var promo = d.promocion || d;
    var descuento = d.descuento || 0;
    var descripcion = d.descripcion || promo.descripcion || promo.nombre || code;
    d.descripcion = descripcion;
    promoApplied = { descuento: descuento, descripcion: descripcion, id_promocion: d.id_promocion || promo.id_promocion || 0 };
    $('promo-tag').innerHTML = '<span class="promo-tag"><i class="fas fa-tag"></i> ' + esc(descripcion) + ' (-' + fm(descuento) + ')</span>';
    $('promo-input').value = '';
    renderCart();
...
function buscarClientes(q) {
  ...
  _clDebounce = setTimeout(function() {
    ...
    apiGet({ accion: 'clientes', q: q }, function (d) {
      var clientes = d.clientes || d || [];
      list.innerHTML = clientes.map(function (c) {
        return '<div class="cart-item" style="cursor:pointer;" onclick="selectClient(' + c.id_cliente + ',\'' + esc(c.nombre) + '\',\'' + esc(c.rut) + '\',\'' + esc(c.correo) + '\',\'' + esc(c.telefono) + '\')"><div><strong>' + esc(c.nombre) + '</strong><br><span style="font-size:11px;color:#64748b;">' + esc(c.rut) + ' · ' + esc(c.correo) + '</span></div></div>';
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    for (var j = 0; j < cart.length; j++) {
      if (cart[j].id === it.id_producto) {
        lines += '<div class="rec-line"><span>' + esc(cart[j].name) + ' x' + it.cantidad + '</span><span>' + fm(it.precio * it.cantidad) + '</span></div>';
        break;
      }
    }
  }
...
function showPromoManager() {
  ...
  var canCreatePromo = hasPermission('pos', 'crear_promocion');
  var promoButton = canCreatePromo ? '<div style="margin-bottom:10px;"><button class="btn-p" style="padding:8px 16px;font-size:13px;" onclick="closeModal();showNewPromo()"><i class="fas fa-plus"></i> Nueva Promocion</button></div>' : '';
...
function savePromo() {
  if (!hasPermission('pos', 'crear_promocion')) { toast('No tiene permiso para crear promociones', 'err'); return; }
...
function elimPromo(id) {
  if (!hasPermission('pos', 'crear_promocion')) { toast('No tiene permiso para eliminar promociones', 'err'); return; }
...
function saveCotizacion() {
  var items = cart.map(function (c) { return { id_producto: c.id, producto: c.name, sku: c.sku, cantidad: c.cant, precio: c.price, precio_unitario: c.price }; });
  apiPost({
...
function showCotizacionesList() {
  apiGet({ accion: 'cotizaciones' }, function (d) {
    var cotizaciones = d.cotizaciones || d || [];
