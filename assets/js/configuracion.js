...
function apiCfg(accion, data, cb) {
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState === 4) {
      try { var d = JSON.parse(xhr.responseText); if (xhr.status >= 200 && xhr.status < 300) { if (cb) cb(d); } else { toast(d.message || d.mensaje || d.error || 'Error', 'error'); } }
...
function apiBoleta(method, data, cb) {
  var xhr = new XMLHttpRequest();
  var url = '../assets/api/pos.php';
  if (method === 'GET') url += '?action=config_boleta';
    ...
    if (xhr.readyState === 4) {
      try {
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          toast(d.message || d.mensaje || d.error || 'Error', 'error');
        }
      } catch (e) {
        toast('Error de respuesta', 'error');
      }
    }
  };
...
function cargarConfigBoleta() {
  apiBoleta('GET', null, function(res) {
    var d = res.config || res || {};
...
function guardarConfigBoleta(event) {
  ...
  data.action = 'config_boleta_guardar';
  apiBoleta('POST', data, function(r) {
    toast(r.message || 'Configuracion de boleta guardada');
  });
}
