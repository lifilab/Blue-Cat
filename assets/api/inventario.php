...
function invNumero($valor): ?float {
    if ($valor === null || $valor === '') {
        return null;
    }
    ...
    $hasComma = strpos($valor, ',') !== false;
    $hasDot = strpos($valor, '.') !== false;
...
case 'precio_actualizar':
    requierePermiso('inventario','editar');
    $id = (int)($input['id_producto'] ?? $input['id'] ?? 0);
    $precio = invNumero($input['precio_venta'] ?? $input['precio'] ?? null);
    if (!$id) json(['ok'=>false, 'error'=>'Producto requerido'],400);
    if ($precio === null || $precio < 0) json(['ok'=>false, 'error'=>'Precio invalido'],400);

    $usuariosCuenta = getUsuariosCuentaIds($conn, $uid);
    $inUsuarios = implode(',', array_fill(0, count($usuariosCuenta), '?'));
    $types = 'i' . str_repeat('i', count($usuariosCuenta));
    $params = array_merge([$id], $usuariosCuenta);

    $conn->begin_transaction();
    try {
        $stmt = $conn->prepare("SELECT id_producto, nombre_producto, precio_venta FROM producto WHERE id_producto=? AND id_user IN ($inUsuarios) FOR UPDATE");
        $stmt->bind_param($types, ...$params);
        $stmt->execute();
        $r = $stmt->get_result();
        ...
        if (!$producto) {
            $conn->rollback();
            json(['ok'=>false, 'error'=>'Producto no encontrado o sin permiso'],404);
        }

        $precioAnterior = (float)($producto['precio_venta'] ?? 0);
        $stmt = $conn->prepare("UPDATE producto SET precio_venta=? WHERE id_producto=?");
        $stmt->bind_param('di', $precio, $id);
        $stmt->execute();
        $stmt->close();

        invLog($conn, $uid, 'ACTUALIZAR_PRECIO', 'producto', $id, [
        ...
        $conn->rollback();
        json(['ok'=>false, 'error'=>'No se pudo actualizar el precio'],500);
    }
...
case 'stock_actualizar':
    ...
    $campo = $input['campo'] ?? '';
    $valor = invNumero($input['valor'] ?? null);
    $allowed = ['disponible','reservado','comprometido','en_transito','danado','bloqueado','devuelto','produccion'];
    if (!$id_stock || !in_array($campo, $allowed, true) || $valor === null || $valor < 0) json(['error'=>'Datos invalidos'],400);
    ...
    $stmt2->execute();
    $r = $stmt2->get_result();
    $stockRow = $r->fetch_assoc();
    $stmt2->close();
    if (!$stockRow) json(['error'=>'No autorizado'], 403);
    $stmt2 = $conn->prepare("UPDATE stock SET $campo=? WHERE id_stock=?");
    $stmt2->bind_param("di", $valor, $id_stock);
    $stmt2->execute();
    $stmt2->close();
    if ($campo === 'disponible') {
        $idProductoStock = (int)$stockRow['id_producto'];
        $stmt2 = $conn->prepare("SELECT COALESCE(SUM(disponible),0) AS total FROM stock WHERE id_producto=?");
        $stmt2->bind_param("i", $idProductoStock);
        $stmt2->execute();
        $totalRow = $stmt2->get_result()->fetch_assoc();
        $stmt2->close();
        $total = (float)($totalRow['total'] ?? 0);
        $stmt2 = $conn->prepare("UPDATE producto SET cantidad=? WHERE id_producto=?");
        $stmt2->bind_param("di", $total, $idProductoStock);
        $stmt2->execute();
        $stmt2->close();
    }
...
case 'stock_set':
    requierePermiso('inventario','editar');
    $id_producto = (int)($input['id_producto'] ?? $input['id'] ?? 0);
    $id_bodega = (int)($input['id_bodega'] ?? 0);
    $stockNuevo = invNumero($input['stock'] ?? $input['cantidad'] ?? $input['valor'] ?? null);
    if (!$id_producto) json(['ok'=>false, 'error'=>'Producto requerido'],400);
    if ($stockNuevo === null || $stockNuevo < 0) json(['ok'=>false, 'error'=>'Stock invalido'],400);
    if (!$id_bodega) {
        $id_bodega = getDefaultBodega($conn);
    }
    if (!$id_bodega) json(['ok'=>false, 'error'=>'No existe bodega disponible'],400);

    $usuariosCuenta = getUsuariosCuentaIds($conn, $uid);
    $inUsuarios = implode(',', array_fill(0, count($usuariosCuenta), '?'));
    $types = 'i' . str_repeat('i', count($usuariosCuenta));
    $params = array_merge([$id_producto], $usuariosCuenta);

    $conn->begin_transaction();
    try {
        $stmt = $conn->prepare("SELECT id_producto, nombre_producto, cantidad FROM producto WHERE id_producto=? AND id_user IN ($inUsuarios) FOR UPDATE");
        $stmt->bind_param($types, ...$params);
        $stmt->execute();
        $r = $stmt->get_result();
        $producto = $r->fetch_assoc();
        ...
        $stmt = $conn->prepare("SELECT id_stock, disponible FROM stock WHERE id_producto=? AND id_bodega=? LIMIT 1 FOR UPDATE");
        $stmt->bind_param("ii", $id_producto, $id_bodega);
        $stmt->execute();
        $stockRow = $stmt->get_result()->fetch_assoc();
        $stmt->close();

        $stockAnterior = $stockRow ? (float)$stockRow['disponible'] : 0.0;
        ...
        if (abs($diferencia) > 0.000001) {
            ...
            actualizarKardex($conn, $uid, $id_producto, $id_bodega, 'AJUSTE', $id_producto, 'STOCK_SET', (int)round($entrada), (int)round($salida), 0, 'Actualizacion directa de stock');
        }

        $stmt = $conn->prepare("SELECT cantidad FROM producto WHERE id_producto=?");
        $stmt->bind_param("i", $id_producto);
        $stmt->execute();
        $productoActualizado = $stmt->get_result()->fetch_assoc();
        $stmt->close();

        invLog($conn, $uid, 'ACTUALIZAR_STOCK', 'producto', $id_producto, [
            'producto' => $producto['nombre_producto'] ?? '',
            'id_bodega' => $id_bodega,
        ...
        json([
            ...
            'producto'=>[
                ...
                'id_bodega'=>$id_bodega,
                'stock'=>$stockNuevo,
                'cantidad'=>(float)($productoActualizado['cantidad'] ?? $stockNuevo)
            ]
        ]);
    } catch (Throwable $e) {
        $conn->rollback();
        json(['ok'=>false, 'error'=>'No se pudo actualizar el stock'],500);
    }
