...
// ═══ USUARIOS ═══
case 'usuarios':
    $items = []; $r = $conn->query("SELECT u.id_user, u.id_empresa, u.nombre, u.correo, u.telefono, u.activo, u.fecha_creacion, u.ultimo_acceso, GROUP_CONCAT(r.nombre SEPARATOR ', ') AS roles FROM usuario u LEFT JOIN usuario_rol ur ON u.id_user=ur.id_user LEFT JOIN rol r ON ur.id_rol=r.id_rol GROUP BY u.id_user, u.id_empresa, u.nombre, u.correo, u.telefono, u.activo, u.fecha_creacion, u.ultimo_acceso ORDER BY u.nombre");
    while ($f = $r->fetch_assoc()) $items[] = $f;
    json($items);
    break;
...
case 'usuario_crear':
    ...
    $stmt = $conn->prepare("INSERT INTO usuario (nombre, correo, password, nombre_completo, cargo, telefono, id_sucursal, validar_sesion) VALUES (?,?,?,?,?,?,?,1)");
    $stmt->bind_param("ssssssi", $nombre, $correo, $hash, $nombre_completo, $cargo, $telefono, $id_sucursal);
    $stmt->execute(); $id = (int)$conn->insert_id; $stmt->close();
