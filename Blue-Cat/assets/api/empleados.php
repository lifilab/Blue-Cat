...
function listEmpleados($conn, $uid) {
    ...
    $sql = "SELECT e.*, (SELECT u.id_user FROM usuario u WHERE u.id_empleado=e.id_empleado LIMIT 1) AS credencial_id_user, (SELECT COUNT(*) FROM empleado_documento WHERE id_empleado = e.id_empleado) as docs FROM empleado e" . $where . " ORDER BY e.apellidos ASC, e.nombres ASC LIMIT ? OFFSET ?";
...
function getEmpleado($conn, $uid, $id) {
    ...
    $stmt->close();
    $e['credencial_id_user'] = $credencial ? (int)$credencial['id_user'] : 0;
...
function crearEvaluacion($conn, $uid, $input) {
    ...
    $stmt->bind_param("issiiiiiiiisss", $id_emp, $periodo, $fecha, $comp, $obj, $prod, $teq, $punt, $resp, $cal, $total, $comentarios, $plan, $eval);
    $stmt->execute();
...
function crearCredenciales($conn, $uid, $input) {
    ...
    $stmt->close();
    if ($existing) json(['error'=>'El empleado ya tiene credenciales vinculadas (user #'.$existing['id_user'].')'], 400);
    ...
    try {
        ...
        $username = $baseUsername; $suffix = 2;
        $checkUser = $conn->prepare("SELECT id_user FROM usuario WHERE nombre = ? LIMIT 1");
...
function eliminarCredenciales($conn, $uid, $input) {
    ...
    try {
        ...
        $conn->rollback();
        json(['error'=>'La cuenta tiene movimientos históricos y no puede eliminarse completamente. Desactívela para conservar la trazabilidad.'],409);
    }
}
