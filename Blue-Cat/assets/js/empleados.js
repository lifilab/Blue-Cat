...
function renderTable(d){
  ...
  for(var i=0;i<d.length;i++){
    ...
    tr.innerHTML=
      ...
      '<td>'+
      (r.credencial_id_user ? '<span onclick="event.stopPropagation();showCrearCredenciales('+r.id_empleado+')" style="display:inline-block;background:#dcfce7;color:#16a34a;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600;margin-right:4px;cursor:pointer;" title="Gestionar credenciales"><i class="fas fa-key"></i></span>' : '')+
...
function showEdit(id){
  ...
  xhr.onload=function(){
    ...
    var panel=m.querySelector('.mc'); if(panel) panel.classList.add('wide-edit');
    m.classList.add('show');
    m.onclick=function(e){if(e.target===m){m.classList.remove('show');if(panel)panel.classList.remove('wide-edit');}};
  };
  xhr.send();
}
...
function showCrearCredenciales(id_empleado) {
  ...
  xhr.onload = function() {
    ...
    var emp = JSON.parse(xhr.responseText);
    var m = $('cm'), b = $('cbody');
    if (emp.credencial_id_user) {

      b.innerHTML =
        '<h3><i class="fas fa-key" style="color:#059669;"></i> Gestionar credenciales de '+esc(emp.nombres)+' '+esc(emp.apellidos)+'</h3>'+
...
function eliminarCredenciales(idEmpleado) {
  if(!confirm('¿Eliminar definitivamente esta cuenta, su correo y sus roles? Esta acción no se puede deshacer.'))return;
  apiPost({accion:'eliminar_credenciales',id_empleado:idEmpleado},function(){
    toast('Cuenta eliminada completamente');$('cm').classList.remove('show');loadEmp();
  });
}
